...
must_haves:
  truths:
    ...
    - "Each card displays session stats: avg visits/session and returning visitor percentage"
    - "All metrics viewable on one screen without clicking between clients"
    - "Cards stack vertically on mobile"
    - "Data refreshes on 30-second polling interval"
    - "Page works correctly when Shrike Media Website client is selected in sidebar"
    - "Visits with NULL website_label do not cause errors or break rendering"
  artifacts:
...
- **Session stats (REQUIRED -- do NOT omit):**
  - Avg visits/session: calculated as `totalVisits / uniqueSessions` (where uniqueSessions is the count of distinct session_id values). Display as a decimal rounded to 1 place (e.g., "2.3").
  - Returning visitor %: calculated as `(totalVisits - uniqueIPs) / totalVisits * 100`. Display as a percentage (e.g., "34.2%"). Handle the zero-visits case (display "0%" or "N/A", do not divide by zero).
...
IMPORTANT: Carry over ALL the existing metric computation logic from the current page -- the location counting, page counting, referrer counting, daily stats, AND session stats calculations. Do not simplify or remove any of them. The only change is scoping each query with website_label.

**4. NULL website_label handling:**
Because the website_label column is nullable (future visits from the external tracker may arrive without a label), the addSiteFilters function uses `.eq('website_label', websiteLabel)` which will only match rows where website_label equals the specific label. Rows with NULL website_label will naturally be excluded from per-site cards -- this is correct and expected behavior.

Do NOT add special "Untagged" handling or a third card. Untagged visits are a known follow-up item (the external tracking script needs updating). For now, per-site cards only show tagged visits, which is the complete set of migrated historical data.

If in the future an "All Visits" aggregate view is needed, it would be a separate concern. For now, the two cards are sufficient.

**5. Restructure the page layout:**
- Keep the title as "Visitor Analytics" to stay consistent with the rest of the app. The sidebar already shows which client is selected.
...
**7. Keep the StatCard component** at the bottom of the file (it's a small internal component). No changes needed to it.
...
2. Read the file and verify:
   ...
   - WebsiteCard component fetches all 6 metric categories independently per site (overview, locations, pages, referrers, session stats, time series)
   - Session stats section exists and computes:
     - avg visits/session as `totalVisits / uniqueSessions`
     - returning visitor % as `(totalVisits - uniqueIPs) / totalVisits * 100`
   ...
   - No special handling for NULL website_label (naturally excluded by .eq filter)
  </verify>
  <done>
Visits page renders two WebsiteCard components in a responsive grid. Each card shows full metrics (visits, IPs, sessions, locations, pages, referrers, session stats including avg visits/session and returning visitor %, time series) for its specific website, filtered by website_label. TypeScript compiles without errors. NULL website_label visits are gracefully excluded from per-site cards without errors.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    3. Select "Shrike Media Website" in the sidebar
    ...
    6. Each card should show: overview stats (visits, unique IPs, sessions), locations, top pages, referrers, session stats (avg visits/session, returning visitor %), and time series chart
...
- Each card has all metric sections from the original page including session stats
- Session stats correctly compute avg visits/session and returning visitor %
...
- NULL website_label visits excluded without errors
</verification>

<success_criteria>
- Visits panel shows each webpage's full metrics in its own distinct card
- All metrics viewable on one screen without clicking between clients
- Session stats (avg visits/session, returning visitor %) present in each card
- Per-site data is correctly filtered by website_label
- Layout is responsive (2-col on lg, 1-col on mobile)
- No TypeScript compilation errors
- No errors when visits with NULL website_label exist in the database
</success_criteria>
