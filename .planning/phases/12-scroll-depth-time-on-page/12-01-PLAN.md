...
must_haves:
  truths:
    ...
    - "Scroll events are sent to Nessus CRM with event_name='scroll_depth' and event_data containing percent_scrolled and page_path"
    - "Observer and sentinel elements are cleaned up on unmount (no memory leaks)"
  artifacts:
  ...
  key_links:
    - from: "useScrollDepth (inside useNessusTracking.ts)"
      to: "trackEvent"
      via: "calls trackEvent('scroll_depth', { percent_scrolled, page_path })"
...
3. Create sentinel elements at 25%, 50%, 75%, 90%, 100% depth:
   - CRITICAL: `position: absolute; top: X%` requires a positioned ancestor. Since body may not have `position: relative`, calculate pixel positions instead: `const pixelTop = (percent / 100) * document.documentElement.scrollHeight`
   - Use `position: absolute; top: ${pixelTop}px; height: 1px; width: 1px; pointer-events: none; visibility: hidden;`
