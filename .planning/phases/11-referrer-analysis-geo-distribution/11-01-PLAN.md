...
must_haves:
  truths:
    ...
    - "Referrer analysis shows engagement quality per source — defined as average event count per session, where each session is assigned to the referrer category of its first event, grouped by referrer source"
...
<task type="auto">
  ...
  <action>
    ...
    These columns already exist in the visits table (pre-verified by orchestrator via information_schema query). No migration needed.
...
<task type="auto">
  ...
  <action>
    ...
    **Referrer categorization logic (in useMemo):**
    Parse each visit's referrer field and categorize into source types. Apply rules in this STRICT PRECEDENCE ORDER (first match wins):

    1. Check tracking params FIRST: if referrer URL contains 'fbclid', categorize as "Facebook" (handles cases like `shrike.vercel.app/page?fbclid=...` which is a Facebook click-through, not self-referral)
    2. Check social domains: if referrer contains 'instagram.com' or 'l.instagram.com', categorize as "Instagram"; if referrer contains 'facebook.com' or 'm.facebook.com', categorize as "Facebook"
    3. Check self-referral: if referrer contains 'shrike.vercel.app', categorize as "Self-referral"
    4. Check null/empty: if referrer is null or empty string, categorize as "Direct"
    5. Fallback: anything else is "Other"

    Use case-insensitive matching via .toLowerCase(). Simple string matching with .includes() — no URL constructor needed.
    ...
    **Engagement quality per source (key differentiator):**
    ...
    2. For each session, determine the referrer category from the FIRST event in the session (earliest created_at — this is the landing page event)
    3. Count total events per session
    4. Group sessions by their referrer category, then average events/session within each category
    This metric shows which traffic sources produce the most engaged visitors (more events = deeper engagement).
  ...
  <done>
    ReferrerAnalysis.tsx exists, exports ReferrerAnalysis component, categorizes referrers into 5 source types with strict precedence (fbclid > social domains > self-referral > null=Direct > Other), shows engagement quality (avg events/session) per source, handles null referrers as "Direct", and compiles without errors.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **US State abbreviation map:**
    Define a `const US_STATE_ABBREV: Record<string, string>` inside the component file (above the component function) mapping all 50 US state names to 2-letter codes, e.g.:
    ```ts
    ...
    ```
    Include DC as well ('District of Columbia': 'DC').

    Create a helper function `formatCityRegion(city: string, region: string | null): string` that returns:
    - If region matches a key in US_STATE_ABBREV (case-sensitive match): "City, XX" (e.g. "Farmville, VA")
    - If region is non-null but not a US state name: "City, Full Region" (e.g. "Toronto, Ontario")
    - If region is null: just "City"

    **Geo computation logic (in useMemo):**
    1. Filter visits where city is non-null (93.5% of data has geo)
    2. Count visits per city (use formatCityRegion for display label)
    ...
    **Rendering:**
    ...
    3. Top cities table: Show top 15 cities sorted by visit count. Each row shows:
       - City name (formatted via formatCityRegion as "City, ST" or "City, Region")
       - Visit count
  ...
  <done>
    GeoDistribution.tsx exists with city/country breakdown and top 15 cities table, using US state abbreviation map for "City, ST" formatting.
...
5. ReferrerAnalysis categorizes referrers into 5 source types with strict precedence (fbclid first) and engagement quality metrics
6. GeoDistribution shows top 15 cities with US state abbreviations and country breakdown
...
- Referrer analysis categorizes all 9 known referrer values into correct source types with fbclid precedence
- Referrer analysis shows engagement quality (avg events/session) per source
- Geo distribution shows "Farmville, VA" as top city with ~231 visits (using state abbreviation map)
