    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="flex gap-4">
          ...
          <label className="flex items-center gap-1 text-sm">
            <input type="radio" name="preferred_contact" value="sms" />
            SMS
          </label>
        </div>
      </div>

      {/* SMS Consent -- only meaningful when preferred_contact is sms, but always visible for simplicity */}
      <div>
...
- SMS consent checkbox is always visible (not conditionally shown based on preferred_contact). The instructions say "only shown when preferred_contact is sms" but implementing that requires controlled state for preferred_contact which adds complexity for minimal benefit in an admin tool where all fields are optional. Keep it simple -- always visible.

**Wait -- re-reading the instructions:** The user explicitly asked for "SMS consent (checkbox, only shown when preferred_contact is sms)." Honor this. Add a small piece of state:

Add `const [preferredContact, setPreferredContact] = useState('email')` to the state declarations.

Update the Preferred Contact radio buttons to be controlled:
```tsx
<input
  type="radio"
...
/>
```
(Same pattern for phone and sms radio buttons.)

Wrap the SMS Consent checkbox in `{preferredContact === 'sms' && (...)}`

Also reset preferredContact when the dialog closes or form resets. In the handleAddLead success path, add `setPreferredContact('email')` after `form.reset()`.

Also add an onClick handler to the Cancel button to reset: `onClick={() => { dialogRef.current?.close(); setPreferredContact('email') }}`.
